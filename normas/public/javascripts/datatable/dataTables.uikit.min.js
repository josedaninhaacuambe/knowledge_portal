...
(function (a) {
  ...
  d.ext.renderer.pageButton.uikit = function (f, l, A, B, m, t) {
    var u = new d.Api(f),
      C = f.oClasses,
      ...
      y = function (q, w) {
        ...
        for (x = w.length; r < x; r++) {
          var k = w[r];
          if (Array.isArray(k)) y(q, k);
          else {
            g = h = "";
            switch (k) {
              case "ellipsis":
                h = '<i class="uk-icon-ellipsis-h"></i>';
                g = "uk-disabled disabled";
                break;
              case "first":
                h = '<i class="uk-icon-angle-double-left"></i> ' + n.sFirst;
                g = 0 < m ? "" : " uk-disabled disabled";
                break;
              case "previous":
                h = '<i class="uk-icon-angle-left"></i> ' + n.sPrevious;
                g = 0 < m ? "" : "uk-disabled disabled";
                break;
              case "next":
                h = n.sNext + ' <i class="uk-icon-angle-right"></i>';
                g = m < t - 1 ? "" : "uk-disabled disabled";
                break;
              case "last":
                h = n.sLast + ' <i class="uk-icon-angle-double-right"></i>';
                g = m < t - 1 ? "" : " uk-disabled disabled";
                break;
              default:
                (h = k + 1), (g = m === k ? "uk-active" : "");
            }
            if (h) {
              var F = a("<li>", {
                class: C.sPageButton + " " + g,
                id:
                  0 === A && "string" === typeof k
                    ? f.sTableId + "_" + k
                    : null,
                ...
                .append(
                  a(
                    -1 != g.indexOf("disabled") || -1 != g.indexOf("active")
                      ? "<span>"
                      : "<a>",
                    {
                      href: "#",
                ...
                .appendTo(q);
              f.oApi._fnBindAction(F, { action: k }, E);
              v++;
            }
          }
        }
    ...
    y(
      a(l)
        .empty()
        .html('<ul class="uk-pagination uk-pagination-right uk-flex-right"/>')
        .children("ul"),
      B
    );
    z &&
