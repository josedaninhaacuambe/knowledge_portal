...
const Conversas = require("../../models/chat");
const fs = require("fs");
const { basicFormat } = require("../../public/javascripts/utils");

exports.findAll = async (req, res) => {
  const conversa = await Conversas.findAll({
    attributes: [
        "sender", 
        "message", 
        "status", 
...
exports.save = async (req, res, next) => {
  try {
    const { sender, message, status, DateTime, sector_id } = req.body;
    const file_name = req.file.path;
    const savedConversas = new Conversas({
        sender,
        message,
        status,
        DateTime,
...
exports.update = async (req, res) => {
  const id = req.params.id;
  const conversa = await Conversas.findByPk(id);
  ...
  try {
    const result = await Conversas.update(newEspecialistaBody, {
      where: { id: id },
    });
    if (result == 1) {
