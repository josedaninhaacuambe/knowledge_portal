...
exports.save = async (req, res, next) => {
  try {
    const { name, supervisor, description, data, classificacao, sector_id } = req.body;
    const file_name = req.file.path;
    const savedJornada = new Jornada({
      name: name,
    ...
    // const pro = await savedProcesso.save();
     ...
     title: "Novo resumo adicionado",
   });
    // const Jornada = await savedLegislation.save();
    // return res.json({ body: Jornada, status: 200 });
  } catch (error) {
...
exports.callPDF6 = function (req, res, next) {
  // return res.json({'hi': 'Man'})
  return res.redirect(
    `http://localhost:3001?q=${req.params.data}&name=${req.query.name}&type=${req.query.file_name}`
  );
};

// PDF View
exports.viewPDF6 = async (req, res) => {
  const document = await Jornada.findOne({
    where: {
