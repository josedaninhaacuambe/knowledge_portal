...
exports.findAll = async (req, res) => {
  const book = await Arquivos.findAll({
    attributes: [
      "id",
      "author",
      "file_name",
...
exports.save = async (req, res, next) => {
  try {
    ...
    // const Arquivos = await savedbook.save();
    // return res.json({ body: Arquivos, status: 200 });
  } catch (error) {
...
exports.viewPDF7 = async (req, res) => {
  const book = await Arquivos.findOne({
    where: {
